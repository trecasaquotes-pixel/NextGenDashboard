  ...
  return new Promise((resolve, reject) => {
    const timer = setTimeout(async () => {
      try {
        ...
        logger.info(`[Store] Saved ${filepath}`);
        resolve();
      } catch (error) {
        logger.error(`[Store] Failed to save ${filepath}`, { error });
        debounceTimers.delete(filepath);
        reject(error);
      }
    }, 300);
...
export async function loadJSON<T>(filepath: string, fallback: T): Promise<T> {
  ...
  try {
    ...
    logger.info(`[Store] Loaded ${filepath}`);
    return JSON.parse(data) as T;
  } catch (error: any) {
    if (error.code === "ENOENT") {
      // File doesn't exist, create it with fallback data
      logger.info(`[Store] Creating ${filepath} with default data`);
      await saveJSON(filepath, fallback);
      return fallback;
    }
    logger.error(`[Store] Failed to load ${filepath}`, { error });
    return fallback;
  }
}
