...
const getFinalTotalPaise = (quotation: any): number =>
  quotation?.totals?.finalTotal ??
  ((quotation as any).grandTotal ?? (quotation as any).amountAfterGst ?? 0);
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/quotations/:id", async (req: any, res) => {
    try {
      ...
      if (!isRenderMode) {
        const currentUserId =
          (req as any).user?.claims?.sub ?? (req as any).user?.sub;
        if (!currentUserId || quotation.userId !== currentUserId) {
  ...
  app.get("/api/analytics/dashboard", isAuthenticated, async (req: any, res) => {
    try {
      ...
      const totalRevenue = acceptedQuotations.reduce(
        (sum, q) => sum + getFinalTotalPaise(q) / 100,
        0,
      );
          ...
          (acc, q) => {
            if (q.updatedAt) {
              ...
              if (qMonthKey === monthKey) {
                acc.revenue += getFinalTotalPaise(q) / 100;
                acc.count += 1;
              }
            }
            return acc;
  ...
  app.get("/api/analytics/quotations", isAuthenticated, async (req: any, res) => {
    try {
      ...
      const valueByStatus = allQuotations.reduce((acc: any, q) => {
        ...
        acc[status].count += 1;
        acc[status].value += getFinalTotalPaise(q) / 100;
        return acc;
      }, {});
      ...
      const categoryAnalysis = allQuotations.reduce((acc: any, q) => {
        ...
        acc[category].totalValue += getFinalTotalPaise(q) / 100;
      ...
      const buildTypeAnalysis = allQuotations.reduce((acc: any, q) => {
        ...
        acc[buildType].totalValue += getFinalTotalPaise(q) / 100;
        return acc;
      }, {});
      ...
      allQuotations.forEach((q) => {
        const value = getFinalTotalPaise(q) / 100;
      ...
      for (let i = 5; i >= 0; i--) {
        ...
        const monthRevenue = acceptedQuotations.reduce((sum, q) => {
          if (q.updatedAt) {
            ...
            const qMonthKey = `${qDate.getFullYear()}-${String(qDate.getMonth() + 1).padStart(2, "0")}`;
            if (qMonthKey === monthKey) {
              return sum + getFinalTotalPaise(q) / 100;
            }
          }
          return sum;
        }, 0);
