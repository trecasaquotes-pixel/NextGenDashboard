...
export function TemplateModal({
  ...
  const getUniqueRooms = () => {
    const interiorRooms = template.rooms
      ...
      .map((r) => r.label);
    const fcRooms = template.rooms.filter((r) => r.tab === "FC" && r.fcLine).map((r) => r.label);
    return {
  ...
  const applyTemplateMutation = useMutation({
    mutationFn: async ({ replaceExisting }: { replaceExisting: boolean }) => {
      ...
      const selectedOptionalRoomKeys = optionalRooms.filter((r) => r.checked).map((r) => r.key);
      ...
      const interiorPromises = template.rooms
        ...
        .flatMap((room) => {
          ...
          if (room.defaultItems!.length > 0) {
            ...
            return [
              apiRequest("POST", `/api/quotations/${quotationId}/interior-items`, {
                quotationId,
                roomType: room.label,
                description: "",
                calc: "LSUM",
      ...
      <Dialog open={open} onOpenChange={handleClose}>
        <DialogContent className="sm:max-w-[500px]" data-testid="dialog-template">
          ...
          {step === "preview" && (
            <div className="space-y-4">
              <div>
                ...
                <div className="text-sm text-muted-foreground">{rooms.interiors.join(", ")}</div>
              </div>
              <Separator />
              <div>
                <h4 className="text-sm font-medium mb-2">False Ceiling Rooms:</h4>
                <div className="text-sm text-muted-foreground">{rooms.fc.join(", ")}</div>
              </div>
            </div>
          )}
          ...
          <DialogFooter className="flex-col sm:flex-row gap-2">
            {step === "preview" && (
              <>
                <Button variant="outline" onClick={handleSkip} data-testid="button-skip-template">
                  Skip
                </Button>
                <Button
                  variant="outline"
            ...
            {step === "mode-select" && (
              <Button variant="outline" onClick={handleClose} data-testid="button-cancel-mode">
                Cancel
              </Button>
            )}
          </DialogFooter>
      ...
      <AlertDialog open={showConfirmReplace} onOpenChange={setShowConfirmReplace}>
        <AlertDialogContent data-testid="dialog-confirm-replace">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will remove all current items in this quote and apply a fresh template. This
