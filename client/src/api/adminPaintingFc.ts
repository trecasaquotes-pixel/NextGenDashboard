...
export async function createPaintingPack(
  data: Omit<NewPaintingPackRow, "bulletsJson"> & { bulletsJson: string[] },
): Promise<PaintingPackRow> {
  const res = await apiRequest("POST", "/api/admin/painting-packs", data);
  return res.json();
}

export async function updatePaintingPack(
  id: string,
  data: Partial<Omit<PaintingPackRow, "id" | "createdAt" | "updatedAt">> & {
    bulletsJson?: string[];
  },
): Promise<PaintingPackRow> {
...
export async function togglePaintingPackActive(
  id: string,
  isActive: boolean,
): Promise<PaintingPackRow> {
...
export async function updateFCCatalogItem(
  id: string,
  data: Partial<Omit<FCCatalogRow, "id" | "key" | "createdAt" | "updatedAt">>,
): Promise<FCCatalogRow> {
  const res = await apiRequest("PUT", `/api/admin/fc-catalog/${id}`, data);
  return res.json();
}

export async function toggleFCCatalogItemActive(
  id: string,
  isActive: boolean,
): Promise<FCCatalogRow> {
